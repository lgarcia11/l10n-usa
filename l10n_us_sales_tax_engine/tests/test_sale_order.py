...
class TestSaleOrderTax(UsTaxBaseTest):
    ...
    def test_us_tax_groups_use_odoo_19_privilege(self):
        """All module groups must belong to the dedicated Odoo 19 privilege."""
        privilege = self.env.ref(
            "l10n_us_sales_tax_engine.res_groups_privilege_us_sales_tax"
        )
        for group_xmlid in (
            "group_us_tax_user",
            ...
            group = self.env.ref(f"l10n_us_sales_tax_engine.{group_xmlid}")
    ...
    def test_calculate_tax_action_as_user_without_tax_groups(self):
        ...
        self.assertFalse(
            salesperson.all_group_ids.filtered(
                lambda group: group.privilege_id
                == self.env.ref(
                    "l10n_us_sales_tax_engine.res_groups_privilege_us_sales_tax"
                )
            ),
        ...
        self.assertTrue(
            order.order_line.tax_ids, "tax must be applied despite no groups"
        )

    def test_exempt_rule_gets_explicit_zero_tax(self):
        """An EXEMPT-category product must get an explicit 0% tax line, not
        empty tax_ids — distinguishes "evaluated and exempt" from "tax
        ...
        order.action_calculate_us_tax()
        self.assertEqual(order.order_line.tax_ids.name, "US Sales Tax - Exempt (0%)")
    ...
    def test_real_zero_rate_state_gets_explicit_zero_percent_tax(self):
        ...
        order.action_calculate_us_tax()
        self.assertEqual(order.order_line.tax_ids.name, "US Sales Tax NY 0%")
        self.assertEqual(order.amount_tax, 0.0)
